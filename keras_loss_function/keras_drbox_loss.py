'''
The Keras-compatible loss function for the DRBox model. Currently supports TensorFlow only.
...
class DRBoxLoss:
    ...
    def smooth_L1_loss(self, y_true, y_pred):
        ...
        Arguments:
            y_true (nD tensor): A TensorFlow tensor of any shape containing the ground truth data.
                In this context, the expected tensor has shape `(batch_size, #boxes, 5)` and
                contains the ground truth bounding box coordinates, where the last dimension
                contains `(cx, cy, w, h, angle)`.
    ...
    def compute_loss(self, y_true, y_pred):
        '''
        Compute the loss of the DRBox model prediction against the ground truth.

        Arguments:
            y_true (array): A Numpy array of shape `(batch_size, #boxes, #classes + 15)`,
                where `#boxes` is the total number of boxes that the model predicts
                per image. Be careful to make sure that the index of each given
                box in `y_true` is the same as the index for the corresponding
                box in `y_pred`. The last axis must have length `#classes + 15` and contain
                `[classes one-hot encoded, 5 ground truth box coordinate offsets, 10 arbitrary entries]`
                in this order, including the background class. The last ten entries of the
                ...
                where the last five entries of the last axis contain the anchor box
            ...
            y_pred (Keras tensor): The model prediction. The shape is identical
                to that of `y_true`, i.e. `(batch_size, #boxes, #classes + 15)`.
                The last axis must contain entries in the format
                `[classes one-hot encoded, 5 predicted box coordinate offsets, 10 arbitrary entries]`.

        Returns:
            A scalar, the total multitask loss for classification and localization.
        '''

        self.neg_pos_ratio = tf.concat(self.neg_pos_ratio, axis=0)
        self.n_neg_min = tf.concat(self.n_neg_min,axis=0)
        self.alpha = tf.concat(self.alpha, axis=0)
        ...
        classification_loss = tf.cast(self.log_loss(y_true[:,:,:-15], y_pred[:,:,:-15]), tf.float32) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.cast(self.smooth_L1_loss(y_true[:,:,-15:-10], y_pred[:,:,-15:-10]), tf.float32) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.cast(tf.reduce_max(y_true[:,:,1:-15], axis=-1), tf.float32) # Tensor of shape (batch_size, n_boxes)
