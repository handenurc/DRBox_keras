...
class DRBoxLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.to_float(self.log_loss(y_true[:,:,:-15], y_pred[:,:,:-15])) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.to_float(self.smooth_L1_loss(y_true[:,:,-15:-10], y_pred[:,:,-15:-10])) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.to_float(tf.reduce_max(y_true[:,:,1:-15], axis=-1)) # Tensor of shape (batch_size, n_boxes)
